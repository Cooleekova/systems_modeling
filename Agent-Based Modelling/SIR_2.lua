...
ABMSIR = Model{
    ...
    init = function(model)
        model.agent = Agent{
            ...
            on_message = function(agent)
                agent:getInfected()
            end,
            execute = function(agent)
                if agent.state == "infected" then
                    forEachConnection(agent, function(connection)
                        if connection.state == "susceptible" and Random{p = model.probability}:sample() then
                            agent:message{receiver = connection, delay=1}
